...
export function LeadHubDashboard({ 
  ...
  // ... (rest of the logic remains the same as in original file - filters, pagination, handlers etc.)
  ...
  const handleMoveToBadData = (leadId: string) => {
    if (confirm('Bạn có chắc chắn muốn chuyển lead này vào data xấu?')) {
      ...
      // Update lead quality to cold
      setLocalLeads(prev => prev.map(lead => 
        lead.id === leadId 
          ? { ...lead, quality: 'Cold/Lạnh', status: 'Data xấu' }
          : lead
      ));
      alert('Đã chuyển lead vào data xấu');
    }
  };
    ...
    <div className="flex flex-col h-screen bg-background w-full">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Control Bar - UPDATED TO FIX LAYOUT ISSUES */}
          <div className="flex items-center justify-between p-4 border-b border-border bg-muted/20 flex-shrink-0">
            ...
            {/* Right side - Action Buttons + Bulk Actions */}
            <div className="flex items-center gap-2">
              {/* Bulk Actions - Only show when leads are selected - FIXED LAYOUT */}
              {selectedLeads.length > 0 && (
                <div className="mr-2">
                  <BulkActions 
                    selectedCount={selectedLeads.length}
                    selectedCustomers={selectedLeads.map(id => currentLeads.find(l => l.id === id)).filter(Boolean)}
              ...
              {/* Quick Filter Button - FIXED TEXT */}
              <Button
                variant="outline"
                size="sm"
          ...
          <div className="flex items-center justify-between p-4 border-t border-border flex-shrink-0">
            <div className="flex items-center gap-2">
              <span className="text-sm text-muted-foreground">
                Hiển thị {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredLeads.length)} của {filteredLeads.length} leads
              </span>
            </div>
