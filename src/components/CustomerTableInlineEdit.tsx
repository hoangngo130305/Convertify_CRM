...
export function CustomerTableInlineEdit({ 
  customers, 
  selectedCustomers, 
  onSelectCustomer, 
  onSelectAll, 
    ...
    <div className="h-full overflow-x-auto overflow-y-hidden">
          ...
          <thead className="sticky top-0 bg-muted z-10">
            <tr>
              ...
              {pinnedLeftColumns.map((column, index) => (
                <th 
                  key={column.id} 
                  className={`sticky bg-muted/90 p-3 text-left border-b border-border min-w-[120px] transition-colors z-15 border-r-2 border-r-blue-200 ${
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        >
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            <DropdownMenuItem 
                              onClick={() => handleColumnAction(column.id, 'filter')}
              ...
              {regularColumns.map((column) => (
                <th 
                  key={column.id} 
                  className={`relative p-3 text-left border-b border-border min-w-[120px] transition-colors ${
                    columnHover === column.id ? 'bg-muted/50' : ''
                  } ${
                    dragOverColumn === column.id ? 'bg-yellow-100 border-yellow-400' : ''
                  } ${
                    draggedColumn === column.id ? 'opacity-50' : ''
              ...
              {/* Pinned Right Columns - Fixed before Actions */}
              {pinnedRightColumns.map((column, index) => (
                <th 
                  key={column.id} 
                  className={`sticky bg-muted/90 p-3 text-left border-b border-border min-w-[120px] transition-colors z-20 border-l-2 border-l-orange-200 ${ 
                    columnHover === column.id ? 'bg-orange-50' : ''
                  } ${ 
                    dragOverColumn === column.id ? 'bg-yellow-100 border-yellow-400' : ''
                  } ${ 
                    draggedColumn === column.id ? 'opacity-50' : ''
                  }`}
                  style={{ right: `${160 + (pinnedRightColumns.length - index - 1) * 120}px` }}
                  onMouseEnter={() => setColumnHover(column.id)}
                  onMouseLeave={() => setColumnHover(null)}
                  draggable={true}
                  onDragStart={(e) => handleDragStart(e, column.id)}
              ...
              <th className="sticky right-0 bg-muted p-3 text-left border-b border-border min-w-[160px] z-20">
            ...
            {customers.map((customer) => {
              ...
              return (
                <tr key={customer.id} className="border-b border-border hover:bg-muted/50">
                  ...
                  <td className="sticky left-12 bg-background p-3 z-10 min-w-[200px]" style={{ left: '48px' }}>
                    <div
                  ...
                  {pinnedLeftColumns.map((column, index) => (
                    <td 
                      key={column.id} 
                      className="sticky bg-background p-3 z-5 min-w-[120px] border-r-2 border-r-blue-100"
                      style={{ left: `${calculatePinnedLeft(index)}px` }}
                    >
                  ...
                  {pinnedRightColumns.map((column, index) => (
                    <td 
                      key={column.id} 
                      className="sticky bg-background p-3 z-5 min-w-[120px] border-l-2 border-l-orange-100"
                      style={{ right: `${160 + (pinnedRightColumns.length - index - 1) * 120}px` }}
                    >
                      {renderEditableCell(customer, column)}
                    </td>
                  ...
                  <td className="sticky right-0 bg-background p-3 z-10 min-w-[160px]">
                    <div className="flex items-center justify-center">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          ...
                          {/* Removed "Chuyển Hot" functionality */}
                          
                          {/* Lead Hub Action - Single Item */}
                          <DropdownMenuItem 
                            onClick={() => onMoveToLeadHub && onMoveToLeadHub(customer.id)}
                          >
                            <Users className="h-4 w-4 mr-2" />
                            Chuyển sang Lead Hub
                          </DropdownMenuItem>                          
                          <DropdownMenuSeparator />
      ...
      {filterPopoverOpen && (
        <ColumnFilterPopup
          ...
          columnName={columns.find(col => col.id === filterPopoverOpen)?.name || ''}
          onApplyFilters={(columnId, filters) => {
            setColumnFilters(prev => ({
              ...prev,
              [columnId]: filters
            }));
          ...
          uniqueValues={getUniqueColumnValues(filterPopoverOpen)}
        />
      )}
      ...
      {editColumnPopupOpen && (
        <ColumnEditPopup
          ...
          column={columns.find(col => col.id === editColumnPopupOpen) || {
            id: editColumnPopupOpen,
            name: '',
            type: 'custom',
            visible: true,
            required: false,
          ...
          onSave={(updatedColumn) => {
            if (onColumnsChange) {
              const newColumns = columns.map(col => 
                col.id === editColumnPopupOpen ? { ...col, ...updatedColumn } : col
              );
              onColumnsChange(newColumns);
            }
